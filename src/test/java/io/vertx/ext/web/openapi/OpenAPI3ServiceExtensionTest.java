...
public class OpenAPI3ServiceExtensionTest extends ApiWebTestBase {
  ...
  private void assertThrow(Runnable r, Class exception) {
    try {
      r.run();
      assertThat(exception.getName() + " not thrown", false).isTrue();
    } catch (Exception e) {
      assertThat(exception.getName() + " not thrown. Thrown: " + e.getClass().getName(), e.getClass().equals(exception)).isTrue();
    }
  }
  ...
  private void assertNotThrow(Runnable r, Class exception) {
    try {
      ...
      assertThat(exception.getName() + " not thrown. Thrown: " + e.getClass().getName(), e.getClass().equals(exception)).isFalse();
    }
  }
  ...
  private void assertNotThrow(Runnable r) {
    try {
      ...
      assertThat(false).as("Exception " + e + " is thrown").isTrue();
    }
  }
